# fixed_results_parser.py
import requests
from bs4 import BeautifulSoup
from datetime import datetime
from database import get_db_connection

def parse_correct_results(date_str):
    """
    Парсит результаты матчей с правильной логикой определения счета
    """
    ...
    for container in match_containers:
        try:
            # Извлекаем названия команд - правильные классы
            ...
            # Ищем ОБЩИЙ счет - это первый блок с классом sc-pvs6fr-0
            score_containers = container.find_all('div', class_='sc-pvs6fr-0')
            if not score_containers:
                continue
                
            # Первый контейнер - общий счет
            main_score_container = score_containers[0]
            home_score_elem = main_score_container.find('div', class_='bAhpay')
            away_score_elem = main_score_container.find('div', class_='hdZfIn')
...
def main():
    print("🎯 ИСПРАВЛЕННЫЙ ПАРСЕР РЕЗУЛЬТАТОВ")
    print("=" * 60)
    
    # Парсим результаты за нужную дату
    results = parse_correct_results("2025-08-23")
